...
def save_user_words(db: sqlite3.Connection, user_login: str, mode: str,
                    comment_count: int, word_counts: dict[str, int]) -> None:
...
def main() -> None:
    ...
    parser.add_argument("--top-users", type=int, default=50, metavar="N",
                        help="コメント数上位 N ユーザを対象にする (default: 50)")
    parser.add_argument("--mode", choices=["A", "B", "C"], default="C",
                        help="SudachiPy 分割モード (default: C)")
    parser.add_argument("--pos", nargs="+", default=DEFAULT_POS,
                        help=f"集計対象品詞 (default: {' '.join(DEFAULT_POS)})")
    parser.add_argument("--sim-threshold", type=float, default=0.1,
                        help="グラフエッジとして採用するコサイン類似度の下限 (default: 0.1)")
    parser.add_argument("--min-df", type=int, default=2,
                        help="語彙行列に含める最低ユーザ数（これ未満のユーザにしか出現しない語を除外）(default: 2)")
    parser.add_argument("--rebuild", action="store_true",
                        help="SQLite キャッシュを削除して全ユーザを再処理する")
    parser.add_argument("--collect-only", action="store_true",
                        help="形態素解析・SQLite 保存のみ行い、グラフ構築はスキップする")
    parser.add_argument("--graph-only", action="store_true",
                        help="形態素解析をスキップしてグラフ構築のみ実行する")
