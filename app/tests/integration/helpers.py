...
def seed_user(db, *, user_id=1, login="testuser", display_name="テストユーザー",
              platform="twitch", profile_image_url=None) -> dict:
    ...
    db.execute(
        ...
        """),
        {"user_id": user_id, "login": login, "display_name": display_name,
         "profile_image_url": profile_image_url, "platform": platform},
    )
    db.commit()
    return {"user_id": user_id, "login": login, "display_name": display_name, "platform": platform}


def seed_vod(db, *, vod_id=100, owner_user_id=1, title="テスト配信",
             url="https://www.twitch.tv/videos/100", youtube_url=None,
             length_seconds=3600) -> dict:
    from sqlalchemy import text
    db.execute(
        text("""
        ...
        """),
        {"vod_id": vod_id, "owner_user_id": owner_user_id, "title": title,
         "url": url, "youtube_url": youtube_url, "length_seconds": length_seconds},
    )
    db.commit()
...
def seed_comment(db, *, comment_id="c001", vod_id=100, commenter_user_id=1,
    ...
    db.execute(
        ...
        """),
        {
            "comment_id": comment_id, "vod_id": vod_id,
            "commenter_user_id": commenter_user_id,
            "commenter_login_snapshot": commenter_login_snapshot,
            "body": body, "offset_seconds": offset_seconds,
            "created_at": naive_utc, "likes": likes, "dislikes": dislikes,
        },
    )
    db.commit()
